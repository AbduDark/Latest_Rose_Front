...
const CreateLesson = ({ isOpen, onClose, onSuccess, courses }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'فشل في إنشاء الدرس');
      }

      onSuccess();
    } catch (error) {
      console.error('Create lesson error:', error);
      setError(error.message || 'حدث خطأ أثناء إنشاء الدرس');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg p-6 w-full max-w-2xl border border-gray-700 max-h-screen overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-white">إضافة درس جديد</h2>
          <button
            onClick={onClose}
